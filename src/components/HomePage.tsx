...
type HomePageProps = {
  onNavigate: (page: 'home' | 'classes' | 'create-class' | 'bulletin' | 'dashboard') => void;
  user: User | null;
  onRequireAuth: () => void;
};

export function HomePage({ onNavigate, user, onRequireAuth }: HomePageProps) {
  const handleNavigateWithAuth = (page: 'create-class', requireAuth = false) => {
    if (requireAuth && !user) {
      onRequireAuth();
    } else {
      onNavigate(page);
          ...
          <Card className="bg-[#ffffff] border-[#a8b892] shadow-lg hover:shadow-xl transition-shadow">
            <CardContent className="p-6 text-center">
              ...
              <Button 
                ...
                className="border-[#556B2F] text-[#556B2F] hover:bg-[#556B2F] hover:text-[#f8f9f6]"
              >
                {user ? 'Create a Class' : 'Sign In to Teach'}
              </Button>
            </CardContent>
          </Card>
        ...
        <div className="max-w-4xl mx-auto text-center px-4">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            ...
            <Button 
              ...
              className="border-[#556B2F] text-[#556B2F] hover:bg-[#556B2F] hover:text-[#f8f9f6]"
            >
              {user ? 'Start Teaching' : 'Sign In to Teach'}
            </Button>
          </div>
        </div>
      </div>
